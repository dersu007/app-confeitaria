...
const numberParser = (minVal: number, errorMsg: string) => 
  z.preprocess((val) => {
    ...
    return Number(val);
  }, z.number().min(minVal, errorMsg));

const ingredienteSchema = z.object({
  nome: z.string().min(3, 'O nome deve ter no mínimo 3 caracteres'),
  unidade_embalagem: z.enum(['g', 'kg', 'ml', 'l', 'un']),
  peso_embalagem: numberParser(0.001, 'A quantidade deve ser maior que zero'),
  preco_embalagem: numberParser(0, 'O preço deve ser positivo'),
  estoque_minimo_unidades: numberParser(0, 'O estoque mínimo deve ser positivo'),
  estoque_atual: numberParser(0, 'O estoque atual deve ser positivo'),
...
export const IngredienteModal = ({ ingrediente, onClose, onSave }: IngredienteModalProps) => {
  ...
  const {
    ...
    defaultValues: {
      ...
      // Se não houver valor, deixamos undefined para o input ficar limpo inicialmente
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-8 space-y-6">
          <div className="space-y-4">
            <div>
              ...
              />
              {errors.nome && <span className="text-[10px] text-error font-bold mt-1 ml-1 block">{errors.nome.message}</span>}
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  step="any"
                  placeholder="Ex: 1000"
                  {...register('peso_embalagem', { valueAsNumber: true })}
                ...
                />
                {errors.peso_embalagem && <span className="text-[10px] text-error font-bold mt-1 ml-1 block">{errors.peso_embalagem.message}</span>}
              </div>
            </div>
            ...
            <div>
              ...
              />
              {errors.preco_embalagem && <span className="text-[10px] text-error font-bold mt-1 ml-1 block">{errors.preco_embalagem.message}</span>}
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                />
                {errors.estoque_atual && <span className="text-[10px] text-error font-bold mt-1 ml-1 block">{errors.estoque_atual.message}</span>}
              </div>

              <div>
                <label className="text-[10px] uppercase font-bold text-on-surface-variant block mb-1 ml-1">
                  Estoque Mínimo (Embalagens)
                </label>
                <input
                  type="number"
                  min="0"
                  step="any"
                  placeholder="Ex: 3"
                  {...register('estoque_minimo_unidades', { valueAsNumber: true })}
                ...
                />
                {errors.estoque_minimo_unidades && <span className="text-[10px] text-error font-bold mt-1 ml-1 block">{errors.estoque_minimo_unidades.message}</span>}
                
                {isStaleMinStock ? (
                  <p className="text-[9px] text-error font-bold mt-1 ml-1 animate-pulse">
                    ⚠️ Preencha o peso da embalagem para calcular o mínimo.
                  </p>
                ) : Number(pesoEmbalagem) > 0 && Number(minUnidades) > 0 ? (
                  <p className="text-[9px] text-primary font-bold mt-1 ml-1 italic">
